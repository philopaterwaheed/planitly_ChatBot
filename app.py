...
async def ask_ai(
    ...
    instructions += (
        ...
        "When adding data to Financial Tracker's Income or Expenses arrays, use these EXACT rules:\n\n"
        
        "**CRITICAL: Always use push_at index 0 (NOT append) for Financial Tracker**\n"
        "- Operation: 'push_at' (pushes to beginning of array)\n"
        "- Index: 0 (always push to the front)\n"
        "- Format: {'item': {'key': 'description', 'value': 'amount;date'}}\n"
        "- Date format: YYYY-MM-DD preferred\n\n"
        
        "**Financial Tracker Data Structure:**\n"
        "- Income and Expenses are Array_of_pairs components\n"
        "- Each entry has: key (description) and value (amount;date)\n"
        "- New entries are always added at the beginning (index 0)\n"
        "- This keeps the most recent transactions at the top\n\n"
        
        "**Correct Financial Tracker Examples:**\n"
        "User: 'I got paid $2000 today for my salary'\n"
        "→ create_data_transfer(\n"
        "    target_component_id='income_component_id',\n"
        "    operation='push_at',\n"
        "    data_value={'item': {'key': 'Salary', 'value': '2000;2025-06-08'}, 'index': 0}\n"
        "  )\n\n"
        
        "User: 'I spent $75 on gas yesterday'\n"
        "→ create_data_transfer(\n"
        "    target_component_id='expenses_component_id',\n"
        "    operation='push_at',\n"
        "    data_value={'item': {'key': 'Gas', 'value': '75;2025-06-07'}, 'index': 0}\n"
        "  )\n\n"
        
        "User: 'Add $500 freelance income from January 10th'\n"
        "→ create_data_transfer(\n"
        "    target_component_id='income_component_id',\n"
        "    operation='push_at',\n"
        "    data_value={'item': {'key': 'Freelance Work', 'value': '500;2025-01-10'}, 'index': 0}\n"
        "  )\n\n"
        
        "**WRONG Examples (DO NOT USE):**\n"
        "operation='append' (use push_at instead)\n"
        "data_value={'item': '500;2025-01-10'} (missing key-value structure)\n"
        "index=1 or any index other than 0 (always use index 0)\n"
        "{'item': {'value': '500;2025-01-10'}} (missing 'key' field)\n\n"
        
        "**Description Generation Rules:**\n"
        "- If user provides description: use it as the 'key'\n"
        "- If no description provided: generate appropriate description\n"
        "  • Income: 'Salary', 'Freelance', 'Bonus', 'Other Income'\n"
        "  • Expenses: 'Gas', 'Food', 'Utilities', 'Shopping', 'Other Expense'\n"
        "- Keep descriptions concise and clear\n\n"
        
        "**Date Handling:**\n"
        "- 'today': Use current date (2025-06-08)\n"
        "- 'yesterday': Calculate previous day\n"
        "- Specific dates: Parse and format as YYYY-MM-DD\n"
        "- No date provided: Use current date\n\n"
        
        "**Financial Tracker Component Identification:**\n"
        "- Look for subject with name containing 'Financial' or 'Finance'\n"
        "- Find components named 'Income' and 'Expenses' (or similar)\n"
        "- These should be Array_of_pairs type components\n"
        "- Always verify component IDs from the ai_accessible_subjects data\n\n"
